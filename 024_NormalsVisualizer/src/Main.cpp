...
std::string const VERT_NORMAL_SHADER { "shaders/vert/normalDisplayShader.glsl" };
std::string const GEO_NORMAL_SHADER { "shaders/geo/normalDisplayShader.glsl" };
std::string const FRAG_NORMAL_SHADER { "shaders/frag/normalDisplayShader.glsl" };
...
//adjusted as well this is a callback to do that
    ...
    std::string vertNormalShaderPath = (CWD+'/'+ VERT_NORMAL_SHADER);
    std::string geoNormalShaderPath = (CWD+'/'+ GEO_NORMAL_SHADER);
    std::string fragNormalShaderPath = (CWD+'/'+ FRAG_NORMAL_SHADER);
    ShaderProgram normalDisplayShader {vertNormalShaderPath, geoNormalShaderPath, fragNormalShaderPath};
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        normalDisplayShader.setMatrix("projection", cam.getProjectionMatrix());
        normalDisplayShader.setMatrix("view", cam.getViewMatrix()); 
        ...
        for (unsigned int i{0}; i<10; ++i){
            ...
            normalDisplayShader.use();
            normalDisplayShader.setMatrix("model", model);
            obj.draw(normalDisplayShader);
        }
