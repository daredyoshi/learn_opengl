...
std::string CONTAINER_TEXTURE { "textures/container2.png" };
std::string HAPPY_TEXTURE { "textures/awesomeface.png" };
std::string CONTAINER_TEXTURE_SPECULAR { "textures/container2_specular.png" };
...
//adjusted as well this is a callback to do that
    ...
    std::string ContainerTexturePath = (CWD + '/' + CONTAINER_TEXTURE);
    Texture ContainerTexture{ ContainerTexturePath };

    std::string ContainerSpecTexturePath = (CWD + '/' + CONTAINER_TEXTURE_SPECULAR);
    Texture ContanerSpecTexture{ ContainerSpecTexturePath};
    
    std::string happyTexturePath = (CWD + '/' + HAPPY_TEXTURE);
    Texture happyTexture{ happyTexturePath };
    
    for(Material &mat : materials){
        mat.setDiffuseTex1(ContainerTexture);
        mat.setDiffuseTex2(happyTexture);
        mat.setSpecularTex(ContanerSpecTexture);
    }
    ...
    directionalLight.setShaderMaterial(lightShader);
    spotLight.setShaderMaterial(lightShader);

    for (PointLight& pointLight : pointLights){
        pointLight.setShaderMaterial(lightShader);
        std::cout << "point light name " << pointLight.getName() << '\n';
    }
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        for( PointLight &light : pointLights ){
            ...
            lightModel[3] = glm::vec4(light.getPosition(), 1.0);
        ...
        Material mat = materials[static_cast<int>(totalTime * 0.01) % 3];

        mat.setShaderTextures();
