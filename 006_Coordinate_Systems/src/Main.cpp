...
int main()
{
   ...
   float vertices[] = {
    ...
    // we are translating the scene in the opposite direction that we want to move
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        if(glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS){
            model = glm::rotate(model, 0.02f, glm::vec3(1.0f, 0.0f, 0.0f));
        }
        ...
        if(glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS){
            model = glm::rotate(model, -0.02f, glm::vec3(0.0f, 1.0f, 0.0f));
        }
