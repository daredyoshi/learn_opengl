...
// a second frag Shader for another colored tiangle
// this one is RED
...
unsigned int createFragShader(const char* source){
    int success{0};
    char infoLog[512];
...
int main()
{
    ...
    unsigned int yellowFragmentShader = createFragShader(yellowFragmentShaderSource);
    unsigned int redFragmentShader= createFragShader(redFragmentShaderSource);

    unsigned int yellowProgram = createProgram(vertexShader, yellowFragmentShader);
    unsigned int redProgram = createProgram(vertexShader,redFragmentShader );
