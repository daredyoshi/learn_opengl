...
int main()
{
    ...
    // want to render
    //
    // Now we will try rendering a rectangle. It's not smart to have
    // all the vertices for each triangle, so we craete indices for an EBO 
    float vertices[] = {
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        // 
        // The above is if we want to brute force draw triangles. For the rect we
        // want to draw the elements
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
        // the first argument is the mode we want to draw in (just like arrays)
        // second is the amount of elements we'd like to draw (we have 6 indices)
        // third is the type the indices are which is unsigned ints
        // the last is an offset in the array 
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

        // enable this to use wire-frame rendering
