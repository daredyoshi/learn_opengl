...
void drawTwocontainers(ShaderProgram &shader, unsigned int VAO, float scale=1.0);
...
std::string const FRAG_OUTLINE_SHADER { "shaders/frag/outline.glsl" };
...
{    
    ...
    std::string outlineFragShaderPath = (CWD + FRAG_OUTLINE_SHADER);
    ShaderProgram outlineShader(vertexShaderPath, outlineFragShaderPath);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // make sure we don't update the stencil buffer while drawin the floor
        glStencilMask(0x00);
        shader.use();
        ...
        // stencil setup
        // we want to keep the currently sotred value in the stencil buffer 
        // if any of the tests fail
        // if a test succeeds, we want to erplace the stored value with the reference
        // value set in glStencilFunc
        glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
        // all fragments should pass the stencil test
        glStencilFunc(GL_ALWAYS, 1, 0xFF);
        // this is bits for "1" and enables writing 
        // (otherwise you will be writing 0)
        glStencilMask(0xFF); 

        // draw the cubes (this sets those frags to 1 in the stencil buffer)
        shader.use();
        drawTwocontainers(shader, cubeVAO, 1.0f);

        // now we will set gl to ONLY draw in the parts where the stencil is 1
        // this way we only draw the parts outside of the containers
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilMask(0x00); // disable writing to the stencil buffer
        glDisable(GL_DEPTH_TEST); // not needed for the next draw
        outlineShader.use();
        drawTwocontainers(outlineShader, cubeVAO, 1.1f);
        // reset to defaults
        glStencilMask(0xFF);
        glStencilFunc(GL_ALWAYS, 1, 0xFF);
        glEnable(GL_DEPTH_TEST);
...
void drawTwocontainers(ShaderProgram &shader, unsigned int VAO, float scale){
        ...
        model[3] = glm::vec4(-1.0f, 0.0f, -1.0f, 1.0);
        model = glm::scale(model, glm::vec3{scale});
        shader.setMatrix("model", model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        model = glm::mat4(1.0f);
        model[3] = glm::vec4(2.0, 0.0f, 0.0f, 1.0);
        model = glm::scale(model, glm::vec3{scale});
        shader.setMatrix("model", model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
}
